...
*/
 ...
 /**
     ...
     crea un gs1Set  con el caracter '-' en el nodo raiz.
  */
  gs1Set();
  /**
...
/** @brief borra el contenido del gs1Set
*/
void clear();
...
*/
  ...
  /** @brief metodo para buscar un ia
@param[in] it const_iterator a un string (código a insertar)
Buscar un ia en el principio del string
@return ia si esta al principio, en caso contrario devuelve una cadena vacía
*/

  string find_sub(string::const_iterator it);

 /** @brief metodo para guardar los datos del siguiente nodo a insertar
@param[in] it const_iterator a un string (código a insertar)
@return devuelve la información a guardar en el nodo
*/
  pair<string, int> siguiente_dato(string::const_iterator & it);

  /** @brief funcion para incrementar el número de subcódigos que cuelgan de un código
@param[in] n último nodo de un código
@param[in] subcodigos número de subcódigos a incrementar

*/
  void incrementa_subcodigos(tree<pair<string,int>>::node n, int subcodigos);
