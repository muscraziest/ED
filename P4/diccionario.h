...
{  
public:
	typedef pair <T, D> entrada;
	typedef pair<const T, D> value_type;
	typedef unsigned int size_type;
	class iterator;
	class const_iterator;

	/** @brief constructor primitivo. 
	@post define un constructor primitivo con size=0
	*/
	diccionario();
	...
	*/
	diccionario (const diccionario <T,D> & d);
...
*/
/*! Corrección:
	*En la versión 2, hemos mejorado la eficiencia de este método.
*/
	bool insert( const diccionario<T,D>::entrada & e);
...
@param[out] D & referencia a la definicion asociada a la entrada, nos permite modificar la definición
@post  Si s no esta en el diccionario, el size() sera incrementado en 1.
*/
/*! Corrección:
	
	* Tiene que devolver una referencia. Modificamos el código para que devuelva la referencia correctamente.
*/
	D & operator[](const T & s);
...
/*! Corrección:
	...
	*Hemos añadido que en caso de que el elemento no esté en el diccionario muestre un mensaje de error.
*/
  const D & operator[](const T & s) const;
...
*/
/*! Corrección:
	* Cambiamos el código por uno más sencillo. 
*/
  diccionario <T,D> & operator=( const diccionario <T,D> & org);
...
private:
	...
	friend  ostream& operator<< <> (ostream& , const diccionario <T,D> &);
	friend class iterator;
    friend class const_iterator;
    
  
	vector <diccionario<T,D>::entrada> dic; // vector que almacena los elementos del diccionario
	...
	*/
  	...
  	/** @brief Comprueba si un elemento está repetido
  		@param[in] s nombre del meteorito
      	@return true si el elemento está repetido, falso en caso contrario
	*/
  	bool buscar_rep(const T & s) const;
