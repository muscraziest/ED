...
void vectorD<T>::assign(int p, const  T  & t){
  ...
  it = vd.begin();
  bool found = ((*it).first == p);

  while((!found) && (it != vd.end()))
    ++it;
...
void vectorD<T>::resize(int s){
    ...
    typename list<pair<int,T>>::reverse_iterator it,aux;
    it = vd.rbegin();
...
bool vectorD<T>::operator==( const vectorD<T> & x){
  ...
  if((n_ele != x.n_ele) || (v_nulo != x.v_nulo))
    ...
    if(vd.size() != aux.size()){
      ...
      typename list< pair<int,T> >::iterator it_2;
      it_1 = sbegin();
      it_2 = aux.begin();
      iguales = true;
